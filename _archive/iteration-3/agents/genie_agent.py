...
Live mode (iteration 3 Phase 3C): calls a Databricks Genie Space via the
SDK with **per-question conversation rotation** — every turn opens a
fresh conversation, so we never accumulate context and never hit
REQUEST_LIMIT_EXCEEDED / CONTEXT_EXCEEDED_EXCEPTION. Tradeoff: no
follow-up conversational context, which is fine for our supervisor
pattern (each user turn is treated independently anyway).
