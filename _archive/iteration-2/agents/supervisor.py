...
Both `_call_llm` paths are stubbed in this iteration (raise RuntimeError) so
the app runs end-to-end in mock mode via the fallback paths. Wiring real
endpoints is a one-line change in `_call_llm`.
"""
